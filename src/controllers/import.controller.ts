...
import { UserRole, SubscriptionStatus, SubscriptionType } from '@prisma/client';

export const importAbonnes = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    let currentUsersCount = await prisma.user.count({
      where: { companyId },
    });
    ...
    const tempPasswordHash = await bcrypt.hash('BabiTrack@2026', 12);
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      let typeAbonnement: SubscriptionType = SubscriptionType.ALLER_RETOUR;
      if (Object.values(SubscriptionType).includes(typeAbonnementRaw as SubscriptionType)) {
        typeAbonnement = typeAbonnementRaw as SubscriptionType;
      }

      // Valider le statut
      let statut: SubscriptionStatus = SubscriptionStatus.ACTIF;
      if (Object.values(SubscriptionStatus).includes(statutRaw as SubscriptionStatus)) {
        statut = statutRaw as SubscriptionStatus;
      }
        ...
        await prisma.$transaction(async (tx) => {
          ...
          await tx.subscription.create({
            data: {
              ...
              userId: user.id,
              type: typeAbonnement,
              montant: typeAbonnement === SubscriptionType.ALLER_RETOUR ? 15000 : 8000, // montant simulé
              dateDebut: new Date(),
